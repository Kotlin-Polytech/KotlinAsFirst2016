...
fun isPalindrome(str: String): Boolean {
    val palind = str.toLowerCase().filter { it != ' ' }
    for (i in 0..palind.length / 2) {
        if (palind[i] != palind[palind.length - i - 1]) return false
    }
    return true
}
