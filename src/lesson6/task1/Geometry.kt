...
fun diameter(vararg points: Point): Segment {
    ...
    return Segment(points[segpoints.first], points[segpoints.second])
}
...
data class Line(val point: Point, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        ...
        val C1 = -A1 * point.x - point.y * B1
        val A2 = -Math.sin(other.angle)
        val B2 = Math.cos(other.angle)
        val C2 = -A2 * other.point.x - other.point.y * B2
        val x = (B1 * C2 - B2 * C1) / (A1 * B2 - B1 * A2)
        val y = (A2 * C1 - C2 * A1) / (A1 * B2 - B1 * A2)
        return Point(x, y)
...
fun bisectorByPoints(a: Point, b: Point): Line {
    val x = (a.x + b.x) / 2
    val y = (a.y + b.y) / 2
    return Line(Point(x, y), lineBySegment(Segment(a, b)).angle + Math.PI / 2)
