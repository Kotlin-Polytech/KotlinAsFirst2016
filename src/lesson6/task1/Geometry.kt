...
fun circleByDiameter(diameter: Segment): Circle {
    ...
    val c = Point((diameter.begin.x + diameter.end.x) / 2.0, (diameter.begin.y + diameter.end.y) / 2.0)
    return Circle(c, r)
}
...
data class Line(val point: Point, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        ...
        val a = -sinA * point.x - cosA * point.y
        val sinB = -Math.sin(other.angle)
        val cosB = Math.cos(other.angle)
        val b = -sinB * other.point.x - cosB * other.point.y
        val low = sinA * cosB - cosA * sinB
        return Point((b * cosA - a * cosB) / low, (a * sinB - b * sinA) / low)
    }
}
...
fun bisectorByPoints(a: Point, b: Point): Line {
    val x = a.x + (b.x - a.x) / 2.0
    val y = a.y + (b.y - a.y) / 2.0
    return Line(Point(x, y), (lineByPoints(a, b).angle + Math.PI / 2.0) % Math.PI)
}
