...
data class Square(val column: Int, val row: Int) {
     ...
     */
    fun notation() = if (!inside()) "" else ('a' + column - 1) + "$row"
}
...
fun square(notation: String): Square {
    ...
    try {
        ...
        if (!square.inside()) throw IllegalArgumentException()
        return square
 ...
 */
fun rookTrajectory(start: Square, end: Square) = when (rookMoveNumber(start, end)) {
    0 -> listOf(start)
    1 -> listOf(start, end)
 ...
 */
fun bishopMoveNumber(start: Square, end: Square) = when {
    !start.inside() || !end.inside() -> throw IllegalArgumentException()
    start == end -> 0
...
fun bishopTrajectory(start: Square, end: Square) = when (bishopMoveNumber(start, end)) {
    ...
    -1 -> listOf()
        ...
        listOf(start, Square(column, row), end)
    }
}
/**
 * Средняя
